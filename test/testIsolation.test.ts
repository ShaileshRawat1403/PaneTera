...
describe('test processes cannot reach real PaneTera state', () => {
  ...
  it('the npm test preload isolates each process and removes its state on exit', () => {
    ...
    const result = spawnSync(process.execPath, ['--import', PRELOAD, '-e', [
      ...
      's.tokenIsThrowaway = /^panetera-test-/.test(process.env.PORTAL_TOKEN || "");',
      'console.log(JSON.stringify(s));',
    ].join(' ')], { encoding: 'utf8', env: { ...process.env, HOME: box.home, PORTAL_TOKEN: 'inherited-real-looking-token' } });
    ...
    assert.equal(state.tokenIsThrowaway, true, 'the preload replaces any inherited PORTAL_TOKEN');
  ...
  it('importing the server module neither listens, declares connections, nor loads .env', { timeout: 90_000 }, async () => {
    const box = sandbox();
    const port = await freePort();
    fs.writeFileSync(path.join(box.cwd, '.env'), 'PANETERA_DOTENV_PROBE=loaded\n');
    ...
    assert.equal(report.dotenvLoaded, false, 'a test process never loads .env');
    assert.equal(homeUntouched(box), true);
  });

  it('starting the server listens and writes only inside the supplied isolated state', { timeout: 90_000 }, async () => {
    const box = sandbox();
    const port = await freePort();
    ...
    assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
    const report = probeReport(result.stdout) as { listening: boolean; records: unknown[] };
    assert.equal(report.listening, true);
    assert.deepEqual(report.records, [], 'core PaneTera declares no application connections');
    assert.equal(homeUntouched(box), true);
  });
  ...
  it('the entrypoint starts the server itself, as `tsx watch server/index.ts` does', { timeout: 120_000 }, async () => {
    ...
    try {
      ...
      while (Date.now() < deadline && !output.includes(`listening on http://127.0.0.1:${port}`)) {
