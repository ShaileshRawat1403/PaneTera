...
async function main(): Promise<void> {
  ...
  if (mode === 'start') {
    ...
    const registration = await handle.registration;
    const report = { listening: await portOpen(port), registration: registration.map((result) => result.outcome), records: records() };
  ...
  process.stdout.write(`PROBE ${JSON.stringify({ listening: await portOpen(port), records: records() })}\n`);
  process.exit(0);
}
