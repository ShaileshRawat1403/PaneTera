...
function modifierCapability(overrides: Partial<CapabilityCard> = {}): CapabilityCard {
  return {
    capabilityId: 'cap-add-modifier',
    kind: 'tool',
    name: 'blender.add_modifier',
    label: 'Add modifier',
    description: { source: 'schema-derived', text: '' },
    inputSchema: {
      type: 'object',
      required: ['objectId', 'modifierType'],
      properties: { objectId: { type: 'string' }, modifierType: { type: 'string' }, parameters: { type: 'object' } },
    },
    rawDeclaration: {},
    permission: 'proposable',
    enabled: true,
    structuralDigest: 'digest-add-modifier',
    presentationDigest: 'digest-add-modifier',
    ...overrides,
  } as CapabilityCard;
}
...
describe('approval store binds one immutable payload', () => {
  ...
  const propose = (store: CapabilityApprovalStore, args: Record<string, unknown>) => store.propose({
    ...
    capabilityId: 'cap-add-modifier',
    capabilityDigest: 'digest-add-modifier',
    arguments: args,
    displayArguments: args,
  });
  const expected = { connectionId: 'blender', capabilityId: 'cap-add-modifier', capabilityDigest: 'digest-add-modifier' };
  ...
  it('copies the proposal arguments into the approval and approves a proposal only once', () => {
    ...
    const proposal = propose(store, { objectId: 'CanisterBody', modifierType: 'BEVEL' });
    ...
    assert.deepEqual(approval.arguments, { objectId: 'CanisterBody', modifierType: 'BEVEL' });
  ...
  it('claims without caller arguments, and rejects caller arguments that differ', () => {
    const store = new CapabilityApprovalStore(dir);
    const approval = store.approve(propose(store, { objectId: 'Cube', modifierType: 'BEVEL' }).proposalId);
    assert.throws(() => store.claim(approval.approvalId, { ...expected, arguments: { objectId: 'Other', modifierType: 'BEVEL' } }), /changed after approval/);
    const claim = store.claim(approval.approvalId, expected);
    assert.deepEqual(claim.approval.arguments, { objectId: 'Cube', modifierType: 'BEVEL' });
  });
  ...
  it('refuses to claim an approval whose stored arguments were altered', () => {
    ...
    data.approvals.find((a: { approvalId: string }) => a.approvalId === approval.approvalId).arguments.objectId = 'Tampered';
...
describe('handleProposal validates before queueing', () => {
  ...
  it('rejects a missing required argument with 400, queues nothing, and audits it', () => {
    ...
    const { deps, proposed } = proposalDeps(connectionWith(connectionId, modifierCapability()));
    const result = handleProposal(deps, { connectionId, capabilityId: 'cap-add-modifier', arguments: { modifierType: 'BEVEL' } });
    assert.equal(result.status, 400);
    assert.match(result.payload, /Missing required field: objectId/);
  ...
  it('rejects non-object arguments', () => {
    ...
    const { deps, proposed } = proposalDeps(connectionWith(connectionId, modifierCapability()));
    for (const args of ['objectId=Cube', ['Cube'], null]) {
      assert.equal(handleProposal(deps, { connectionId, capabilityId: 'cap-add-modifier', arguments: args }).status, 400);
    }
    assert.equal(proposed.length, 0);
  });
  ...
  it('stores the validated arguments as both the executable and displayed payload', () => {
    ...
    const { deps, proposed } = proposalDeps(connectionWith(connectionId, modifierCapability()));
    const args = { objectId: 'CanisterBody', modifierType: 'BEVEL' };
    const result = handleProposal(deps, { connectionId, capabilityId: 'cap-add-modifier', arguments: args });
  ...
  it('keeps 404 for a missing target and 403 for a capability that is not proposable', () => {
    ...
    assert.equal(handleProposal(proposalDeps(connectionWith(connectionId, modifierCapability(), 'unreachable')).deps, { connectionId, capabilityId: 'cap-add-modifier', arguments: {} }).status, 404);
    assert.equal(handleProposal(proposalDeps(connectionWith(connectionId, modifierCapability({ permission: 'auto-invocable' } as Partial<CapabilityCard>))).deps, { connectionId, capabilityId: 'cap-add-modifier', arguments: {} }).status, 403);
  });
});
...
describe('handleInvocation executes only the approved arguments', () => {
  ...
  function invocationDeps(connectionId: string, store: CapabilityApprovalStore) {
    ...
    const connection = connectionWith(connectionId, modifierCapability());
  ...
  it('runs the stored arguments when the caller sends none, and refuses caller arguments that differ', async () => {
    ...
    const store = new CapabilityApprovalStore(dir);
    const stored = { objectId: 'CanisterBody', modifierType: 'BEVEL' };
    const proposal = store.propose({ connectionId, capabilityId: 'cap-add-modifier', capabilityDigest: 'digest-add-modifier', arguments: stored, displayArguments: stored });
    const approval = store.approve(proposal.proposalId);
    const { deps, calls } = invocationDeps(connectionId, store);

    const tampered = await handleInvocation(deps, { connectionId, capabilityId: 'cap-add-modifier', approvalId: approval.approvalId, arguments: { objectId: 'Cube', modifierType: 'BEVEL' } });
    assert.equal(tampered.status, 409);
    assert.equal(calls.length, 0, 'differing caller arguments never reach the connector');

    const result = await handleInvocation(deps, { connectionId, capabilityId: 'cap-add-modifier', approvalId: approval.approvalId });
    assert.equal(result.status, 200);
    assert.deepEqual(calls, [{ name: 'blender.add_modifier', args: stored }]);
  });
  ...
  it('refuses an unknown approval before any validation or connector work', async () => {
    ...
    const store = new CapabilityApprovalStore(dir);
    const { deps, calls } = invocationDeps(connectionId, store);
    const result = await handleInvocation(deps, { connectionId, capabilityId: 'cap-add-modifier', approvalId: 'nope', arguments: { objectId: 'Cube', modifierType: 'BEVEL' } });
