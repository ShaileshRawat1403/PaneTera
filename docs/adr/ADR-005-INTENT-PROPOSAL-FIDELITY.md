...
integration (`feat/blender-reaper-integration`). Applies to every Rig
capability, not only the creative integrations.
...
- "Add a bevel" without a target object produces a clarification, not a
  proposal.
...
- Blender and REAPER surface propose actions are hidden until the workstation
  can collect their arguments.
