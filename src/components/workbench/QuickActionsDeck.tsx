...
export const QuickActionsDeck: React.FC<DeckProps> = ({ onTriggerAction }) => {
  ...
  const actions: ActionItem[] = [
    ...
    },
    {
      ...
      icon: <SecurityIcon sx={{ fontSize: 14, color: status.danger }} />,
  ...
  return (
    <Box sx={{ mb: 3 }}>
      <Typography variant="caption" sx={{ color: ink.muted, fontWeight: 800, display: 'block', mb: 1.5, letterSpacing: '0.05em' }}>
        ...
        {actions.map((act) => (
          <Button
            ...
            sx={{
              ...
              borderColor: act.id === 'security-demo' ? status.dangerMuted : surface.border,
              color: act.id === 'security-demo' ? status.danger : ink.primary,
              '&:hover': {
                borderColor: act.id === 'security-demo' ? status.danger : accent.violet,
                backgroundColor: act.id === 'security-demo' ? status.dangerMuted : accent.violetMuted
              }
            }}
          >
            {act.label}
      ...
      >
        <DialogTitle sx={{ color: status.danger, fontWeight: 800, fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: 1 }}>
          <SecurityIcon sx={{ fontSize: 18 }} />
          Simulated Safety Check
        </DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: ink.secondary, fontSize: '0.78rem', lineHeight: 1.5 }}>
        ...
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button size="small" onClick={() => setIsDemoConfirmOpen(false)} sx={{ color: ink.secondary, textTransform: 'none' }}>
            Cancel
          </Button>
          <Button
            size="small"
            ...
            sx={{
              ...
              textTransform: 'none',
              fontWeight: 600,
              '&:hover': { backgroundColor: status.danger }
            }}
          >
            Run safety check
          </Button>
