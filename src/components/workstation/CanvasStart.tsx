...
/** One start tile with warm graphite glassmorphism and subtle micro-interactions. */
  ...
  return (
    <Button
      ...
      sx={{
        ...
        ...(primary
          ? { '&::before': { content: '""', position: 'absolute', left: 0, top: 12, bottom: 12, width: '3px', borderRadius: '3px', backgroundColor: accent.violet } }
          : {}),
        '&:hover': {
    ...
    >
      <Stack direction="row" alignItems="baseline" justifyContent="space-between" gap={1} sx={{ width: '100%' }}>
        <Typography variant="subtitle2" sx={{ color: ink.primary, fontWeight: 650, fontSize: '0.9375rem' }}>
          {title}
        </Typography>
        {hint && (
          <Typography component="span" variant="caption" sx={{ flexShrink: 0, color: ink.muted, fontFamily: typography.mono, fontSize: '0.75rem' }}>
            {hint}
          </Typography>
        )}
      </Stack>
      <Typography variant="caption" sx={{ color: ink.secondary, lineHeight: 1.5, fontSize: '0.8125rem' }}>
        {detail}
      </Typography>
    </Button>
  );
...
export function CanvasStart({ onChooseProject, onConnectCapability, onDescribeGoal }: CanvasStartProps): React.ReactElement {
  return (
    ...
    >
      {/* Pane-seam motif with subtle warmth */}
      <Box aria-hidden sx={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        <Box sx={{ position: 'absolute', left: '50%', top: '18%', bottom: '18%', width: '1px', backgroundColor: surface.border, opacity: 0.4, display: { xs: 'none', md: 'block' } }} />
        <Box sx={{ position: 'absolute', top: '50%', left: '12%', right: '12%', height: '1px', backgroundColor: surface.border, opacity: 0.3, display: { xs: 'none', md: 'block' } }} />
      </Box>
      ...
      >
        {/* Left: the choice and instruction */}
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, ...enterStyles(), animationDelay: '0ms' }}>
          ...
          <Typography variant="body2" sx={{ color: ink.secondary, lineHeight: 1.7, maxWidth: '46ch', fontSize: '0.875rem' }}>
            Whatever you start takes shape here: a live application, a document, a result, or
            evidence you can inspect.
          </Typography>
          <Typography variant="caption" sx={{ mt: 0.5, color: ink.muted, lineHeight: 1.6, fontSize: '0.75rem' }}>
            Describe your goal in the composer, or press{' '}
            <Box component="span" sx={{ px: 0.6, py: 0.1, borderRadius: `${radius.sm}px`, border: `1px solid ${surface.border}`, backgroundColor: surface.sunken, color: ink.secondary, fontFamily: typography.mono, fontSize: '0.75rem' }}>/</Box>
            {' '}for quick actions.
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, ...enterStyles(), animationDelay: '80ms' }}>
          <Typography variant="caption" sx={{ color: ink.muted, fontWeight: 650, letterSpacing: '0.02em', fontSize: '0.75rem' }}>
            Start here
          </Typography>
          <StartAction
            variant="primary"
        ...
        {/* Full width footer caption */}
        <Box sx={{ gridColumn: { md: '1 / -1' }, pt: 1, borderTop: `1px solid ${surface.border}` }}>
          <Typography variant="caption" sx={{ color: ink.muted, lineHeight: 1.6, fontSize: '0.75rem' }}>
